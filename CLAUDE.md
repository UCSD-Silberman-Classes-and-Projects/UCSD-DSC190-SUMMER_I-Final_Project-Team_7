...
- Prior single-line-tracking iterations, in rough chronological order, for reference when reading `LaneFollower`'s history: `LineFollower` (baseline, see below) → `RobustLineFollower` (adds shape filtering and position continuity/tracking on top of the baseline) → `BetterLineFollower` (further shape/color/PID fixes plus sustained-line-loss handling) → `donkeycar/parts/lane_follower2.py` (first lane-aware iteration) → the current `donkeycar/parts/lane_follower.py` (merges further fixes on top — see its module docstring for the complete history). None of the pre-lane-aware ones are lane-aware; they all track exactly one line. (`RobustLineFollower`/`BetterLineFollower`'s source isn't present in this repo or its remote — they predate this checkout.)
